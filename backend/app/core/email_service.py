...
def send_password_reset_email(to_email: str, token: str) -> None:
    if not settings.mail_enabled:
        logger.warning("Email not configured — skipping password reset email to %s. Set MAIL_USER and MAIL_PASSWORD.", to_email)
        return

    reset_url = f"{settings.app_url}/reset-password?token={token}"

    msg = MIMEMultipart("alternative")
    msg["Subject"] = "Reset your Budget password"
    msg["From"] = settings.mail_user
    msg["To"] = to_email

    text = f"Reset your Budget password by visiting: {reset_url}\n\nThis link expires in 1 hour. If you didn't request this, you can safely ignore this email."
    html = f"""
    <html>
    <body style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
      <h2 style="color: #6366f1;">💰 Budget — Reset your password</h2>
